...
# Import blueprints
from blueprints import auth_bp, api_bp, admin_bp, main_bp
...
for var in required_vars:
    value = os.getenv(var)
    logger.info(f"{var}: {'SET' if value else 'NOT SET'} ({'*' * min(len(value) if value else 0, 5) if value else 'None'})")
...
def create_app(config_name: str = None):
    ...
    def handle_csrf_error(e):
        ...
        return render_template('error.html', error="Security token expired. Please refresh the page."), 400
    ...
    set_api_models(services.mapping_model, services.proposal_model, services.cache_model)
