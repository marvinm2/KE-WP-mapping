...
class MetricsCollector:
    def __init__(self, db_path: str = "ke_wp_mapping.db"):
        ...
        self.memory_metrics = defaultdict(lambda: {
    ...
    def init_metrics_table(self):
        ...
        try:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_metrics_endpoint ON metrics(endpoint)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_metrics_status_code ON metrics(status_code)')
    ...
    def record_request(self, endpoint: str, method: str, status_code: int, 
        ...
        threading.Thread(
            ...
            args=(timestamp, endpoint, method, status_code, response_time, 
                  client_ip, user_agent, error_message)
        ).start()
    
    def _store_metric_async(self, timestamp: int, endpoint: str, method: str, 
                           status_code: int, response_time: float,
                           client_ip: str, user_agent: str, error_message: str):
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'error_rate_last_hour': round(total_errors / max(total_requests, 1) * 100, 2),
...
class PerformanceProfiler:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if duration > 1.0:  # Log slow operations
            logger.warning(f"Slow operation detected: {self.operation_name} took {duration:.3f} seconds")
