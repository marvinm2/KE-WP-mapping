...
class Database:
    ...
    def init_db(self):
        ...
        try:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_mappings_ke_id ON mappings(ke_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_mappings_wp_id ON mappings(wp_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_mappings_created_by ON mappings(created_by)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_proposals_mapping_id ON proposals(mapping_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_cache_expires ON sparql_cache(expires_at)')
    ...
    def _migrate_proposals_admin_fields(self, conn):
        ...
        try:
            ...
            admin_fields = ['admin_notes', 'approved_by', 'approved_at', 'rejected_by', 'rejected_at']
            missing_fields = [field for field in admin_fields if field not in columns]
            
            if missing_fields:
                logger.info(f"Adding missing admin fields to proposals table: {missing_fields}")
                
                for field in missing_fields:
                    if field.endswith('_at'):
                        conn.execute(f"ALTER TABLE proposals ADD COLUMN {field} TIMESTAMP")
                    else:
...
class MappingModel:
    ...
    def create_mapping(self, ke_id: str, ke_title: str, wp_id: str, wp_title: str,
                      connection_type: str = 'undefined', confidence_level: str = 'low',
        ...
        try:
            cursor = conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (ke_id, ke_title, wp_id, wp_title, connection_type, confidence_level, created_by))
    ...
    def update_mapping(self, mapping_id: int, connection_type: str = None, 
                      confidence_level: str = None, updated_by: str = None) -> bool:
        """
...
class ProposalModel:
    ...
    def create_proposal(self, mapping_id: int, user_name: str, user_email: str,
                       user_affiliation: str, github_username: str = None,
                       proposed_delete: bool = False, proposed_confidence: str = None,
                       proposed_connection_type: str = None) -> Optional[int]:
        ...
        try:
            cursor = conn.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (mapping_id, user_name, user_email, user_affiliation, github_username,
                  proposed_delete, proposed_confidence, proposed_connection_type))
            
            conn.commit()
            logger.info(f"Created proposal for mapping {mapping_id} by {github_username}")
            return cursor.lastrowid
        except Exception as e:
    ...
    def update_proposal_status(self, proposal_id: int, status: str, admin_username: str = None,
        ...
        try:
            ...
            conn.commit()
            logger.info(f"Updated proposal {proposal_id} to {status} by {admin_username}")
            return True
        except Exception as e:
...
class CacheModel:
    ...
    def cache_response(self, endpoint: str, query_hash: str, response_data: str,
                      expiry_hours: int = 24) -> bool:
        """Cache SPARQL response"""
        # Validate expiry_hours to prevent SQL injection
        if not isinstance(expiry_hours, int) or expiry_hours < 1 or expiry_hours > 168:  # Max 1 week
            expiry_hours = 24
