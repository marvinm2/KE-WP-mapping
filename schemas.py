...
class MappingSchema(Schema):
    ...
    wp_id = fields.Str(
        ...
        validate=[
            ...
            validate.Regexp(
                r'^WP\d+$',
                error="WP ID must be in format 'WPnumber'"
            )
        ]
    )
    wp_title = fields.Str(
...
class ProposalSchema(Schema):
    ...
    deleteEntry = fields.Str(
        missing='',
        validate=validate.OneOf(['', 'on'])
    )
    ...
    def validate_entry_json(self, value):
        ...
        try:
            ...
            for field in required_fields:
                ...
                if not (entry_data.get(field) or entry_data.get(field.replace('_', '').upper())):
